...
export default function ShearMomentDiagram({ flexureCheck, shearCheck, inputs }: Props) {
  ...
  return (
    <div className="space-y-2">
      <h3 className="text-sm font-bold text-slate-200">Bending Moment & Shear Force Diagrams</h3>
      ...
      <div className="rounded-lg bg-slate-900 border border-slate-700 p-3 overflow-x-auto">
        <svg width={W} height={H * 1.5} viewBox={`0 0 ${W} ${H * 1.5}`} className="mx-auto">
          {/* BMD Title */}
          <text x={PAD_L} y={PAD_T - 6} fill="#94a3b8" fontSize="10" fontWeight="600">
            Bending Moment Diagram
          </text>

          {/* BMD baseline */}
          <line x1={PAD_L} y1={PAD_T + innerH} x2={PAD_L + innerW} y2={PAD_T + innerH} stroke="#475569" strokeWidth="1" />
          {/* Support lines */}
          <line x1={PAD_L} y1={PAD_T} x2={PAD_L} y2={PAD_T + innerH} stroke="#475569" strokeWidth="1" />
          <line x1={PAD_L + innerW} y1={PAD_T} x2={PAD_L + innerW} y2={PAD_T + innerH} stroke="#475569" strokeWidth="1" />
          ...
          <line
            ...
            strokeDasharray="4 3"
          />
          <text x={PAD_L + innerW + 2} y={PAD_T + innerH - innerH * 0.9 * (capacityMu / demandMu) + 4} fill="#10b981" fontSize="8">
            φMn
          </text>

          {/* Peak label */}
          <text x={PAD_L + innerW / 2} y={PAD_T + 8} fill="#38bdf8" fontSize="10" textAnchor="middle">
            {demandMu.toFixed(1)} kN·m/m
          </text>

          {/* SFD Title */}
          <text x={PAD_L} y={PAD_T + innerH + 8} fill="#94a3b8" fontSize="10" fontWeight="600">
            Shear Force Diagram
          </text>
          ...
          {/* Left shear label */}
          <text x={PAD_L + 4} y={PAD_T + innerH + 18} fill="#818cf8" fontSize="9">
            +{demandVu.toFixed(1)}
          </text>
          <text x={PAD_L + 4} y={PAD_T + innerH + 20 + innerH * 0.5 + 12} fill="#818cf8" fontSize="9">
          ...
          {/* Span label */}
          <text x={PAD_L + innerW / 2} y={H * 1.5 - 5} fill="#94a3b8" fontSize="10" textAnchor="middle">
            Span: {slabLength}m
          </text>

          {/* Legend */}
          <line x1={PAD_L} y1={H * 1.5 - 20} x2={PAD_L + 16} y2={H * 1.5 - 20} stroke="#38bdf8" strokeWidth="2" />
          <text x={PAD_L + 18} y={H * 1.5 - 16} fill="#94a3b8" fontSize="8">Demand (Mu/Vu)</text>
          <line x1={PAD_L + 110} y1={H * 1.5 - 20} x2={PAD_L + 126} y2={H * 1.5 - 20} stroke="#10b981" strokeWidth="1.5" strokeDasharray="4 3" />
          <text x={PAD_L + 128} y={H * 1.5 - 16} fill="#94a3b8" fontSize="8">Capacity (φMn)</text>
        </svg>
      </div>
    </div>
  );
